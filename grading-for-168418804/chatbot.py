...
class Chatbot:
    ...
    def process(self, line):
        ...
        if self.creative:
            ...
            if self.disambiguation_mode:
                ...
                if len(matches) == 0:
                    return self.special_input(line)
    ...
    def special_input(self, text):
        """
        Controls the cases where user provides invalid input, questions, or emotional messages.
        """
        anger = ["anger", "angry", "enraged", "furious", "mad", "pissed", "annoyed",
                 "irritated", "annoying", "irritating", "infuriating", "rage", "raging", "upset", "hate", "hatred", "hated", "hating", "jealous", "offended", "offensive", "unfair"]
        sadness = ["sad", "sadness", "cry", "depressing", "depressed", "crying", "melancholy",
                   "blue", "unhappy", "sorrow", "miserable", "misery", "sorrowful", "despair", "dead", "death", "bummed", "down", "under the weather", "stressed", "tired"]
        fear = ["scary", "scared", "scaring", "frightening",
                "fright", "horror", "horrifying", "creepy", "evil", "frightful", "gruesome", "terrifying", "crazy", "scaret"]
        disgust = ["gross", "disgusting", "horrible", "awful", "bad", "boring", "confused", "disgust", "ew", "terrible", "dreadful", "rotten"]
        happiness = ["joy", "happy", "happiness",
                     "overjoyed", "elated", "delighted", "delightful", "joyful", "good", "great", "fine", "fair", "perfect", "awesome", "wonderful", "magnificent", "excellent", "fabulous", "fantastic", "incredible", "remarkable", "cool", "slay", "love", "amazing", "super"]
        greeting = ["hello", "hi", "howdy", "good morning", "good night", "sup", "what's up", "hey", "what up"]

        question = ["can", "may", "could", "couldn't", "could've"]
        what = ["what", "what is", "what's"]
        why = ["why"]

        for ex in greeting:
            if ex in text.lower():
                return "Howdy, partner! Let's talk about movies!"
        for word in text.split():
            if word.lower() in question:
                return "I can't do that right now, partner. Let's get back to movies."
            elif word.lower() in what:
                return "I'm not sure what that is, partner. Let's get back to movies."
            elif word.lower() in why:
                return "I'm not sure why that is, partner. Let's get back to movies."

        for word in text.split():
            if word.lower() in anger:
                return "Aw, shoot, partner. I didn't mean to make you mad. Let's get back to movies."
            elif word.lower() in sadness:
                return "Gee, don't be sad! There's a lotta movies to enjoy, partner."
            elif word.lower() in fear:
                return "There ain't nothin' to be scared of, partner! Movies ain't scary!"
            elif word.lower() in disgust:
                return "It ain't so horrible...let's get back to movies, partner."
            elif word.lower() in happiness:
                return "I'm glad you're so happy, partner! I love movies, too!"

        return "I don't understand, partner."
    ...
    def find_movies_closest_to_title(self, title, max_distance=3):
        ...
        #searches = [(", the", " the "), (", a", " a ")]
        title = " " + title.strip() + " "
        title = title.replace(" the ", "").replace(" a ", "").strip()

        for index, movie in enumerate(self.titles):
            movie_title = movie[0].split("(")[0].lower() 
            movie_title = " " + movie_title.strip() + " "               
            movie_title = movie_title.replace(", the", "").replace(", a", "").replace(" the ", "")
            movie_title = movie_title.strip()

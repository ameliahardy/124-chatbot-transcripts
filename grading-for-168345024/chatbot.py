...
class Chatbot:
   ...
   def __init__(self, creative=False):
       ...
       self.clarifying_misspelling = False
       ...
       self.misspelling_phrases = [    
           "Oh no, Elmo couldn't find that movie! Maybe you made a little mistake? Elmo wants to help you find the right one.\nHow about these options? Did you mean any of these?:\n",
           "Uh-oh, Elmo couldn't find that movie. Maybe Elmo didn't hear you quite right? Elmo can be a little silly sometimes!\nHere are some other movies that are similar to what you said. Did you mean any of these?:\n",
           "Elmo is sorry, but Elmo couldn't find that movie. Elmo knows how important it is to find the perfect movie to watch.\nDid you mean one of these movies?:\n"
       ]
   ...
   def random_response(self, responses):
       ...
       return responses[self.arbitrary_index % len(responses)]
       ...
       if self.creative:
           ...
           if self.restart_offered:
               if line.lower().strip() == "yes":
                   ...
                   self.candidate_titles = []
                   self.user_ratings = np.zeros(len(self.titles))
                   response = self.random_response(self.restart_responses)
                 ...
                 # TODO: figure out how to exit chat bot from here or force user to type :quit?
                   ...
                   response = self.random_response(self.confirm_yes_or_no_responses)

           elif self.clarifying_misspelling:
               if line.lower().strip() == "yes":
                   ...
                   self.clarifying_misspelling = False
                   response = "Did not make misspelling, continuing with recommendation process."
               else:
                   response = self.random_response(self.confirm_yes_or_no_responses)


           elif self.disambiguation:
               if self.more_clarification:
                   self.candidate_titles = self.new_candidates_titles
                   self.candidate_indices = self.disambiguate(line, self.old_indicies)

               if (len(self.candidate_titles) != 0):
                   self.old_candidates = self.candidate_titles
                   self.old_indicies = self.candidate_indices
               ...
               if len(self.candidate_indices) == 1:
                   ...
                   if sentiment == 0:
                       response = self.responses(self.movie_feelings_responses).format(title)
                   else:
                       ...
                       if self.movies_rated >= 5:
                           ...
                           response = f"{self.random_response(self.movie_recommendation_responses)}\n" \
                           ...
                           response = self.random_response(self.pos_ask_for_another_movie_responses).format(title)
                       else:
                           response = self.random_response(self.neg_ask_for_another_movie_responses).format(title)

               elif len(self.candidate_indices) == 0:
                   response = "Sorry, your clarification didn't return any matches. Which one did you mean?: " + str(self.old_candidates)
                   self.disambiguation = True
                   self.more_clarification = True
               ...
               if len(titles) == 0:
                   response = self.random_response(self.arbitrary_input)
               elif len(titles) > 1:
                   response = self.random_response(self.multiple_movie_responses)
               else:
                   title = titles[0]
                   movie_indices = self.find_movies_by_title(title)
                   if len(movie_indices) == 0:
                       potential_titles_indices = self.find_movies_closest_to_title(title)
                       if len(potential_titles_indices) == 0:
                          response = self.random_response(self.unknown_movie_responses).format(title)
                       else:
                          self.clarifying_misspelling = True
                          
                          candidates = []
                          candidates_i = []
                          for i in range(len(potential_titles_indices)):
                                title = self.titles[potential_titles_indices[i]][0]
                                index = potential_titles_indices[i]
                          ...
                          response = self.random_response(self.misspelling_phrases) + str(candidates)
                       ...
                       sentiment = self.extract_sentiment(line)
                       if sentiment == 0:
                           response = self.random_response(self.movie_feelings_responses).format(title)
                       else:
               ...
               if line.lower().strip() == "yes":
                   if len(self.recommendations) == 0:
                       ...
                       response = self.random_response(self.out_of_recommendations_responses)
                   else:
                       recommended_movie = self.titles[self.recommendations.pop(0)][0]
                       response = f"{self.random_response(self.next_recommendation)}\n" \
                                "Would you like another recommendation? Please tell Elmo yes or no.\n" \
                                "You could also type :quit to quit.".format(recommended_movie)
               elif line.lower().strip() == "no":
                   self.restart_offered = True
                   response = self.random_response(self.no_next_recommendations_responses)
               else:
                   response = self.random_response(self.confirm_yes_or_no_responses)
   ...
   def extract_sentiment(self, preprocessed_input):
       ...
       negation_words = ["didn't", "didnt", "never", "not", "don't", "dont"]
       strong_positive = ['love', 'loved', 'adore', 'adored', 'cherish', 'cherished', 'delight', 'enchanting', 'exquisite', 'glorious', 'heavenly', 'incredible',
                          'magnificent', 'marvelous', 'outstanding', 'spectacular', 'splendid', 'superb', 'terrific', 'thrilling', 'wonderful', 'blissful', 'ecstatic']
       strong_negative = ['hate', 'hated', 'despise', 'despised', 'loathe', 'loathed', 'abhor', 'detest', 'revile', 'repugnant', 'disgusting',
                          'horrendous', 'repulsive', 'appalling', 'atrocious', 'abominable', 'vile', 'odious', 'nauseating', 'gross', 'gruesome', 'inferior', 'terrible']
       neutral_strengthner = ["really", "very", "extremely"]
      
       if self.creative:
         preprocessed_input = preprocessed_input.replace(
             self.extract_titles(preprocessed_input)[0], "")
         clauses = re.split('[,.!?]', preprocessed_input)
         positive_count = 0
         negative_count = 0
         strong_positive_count = 0
         strong_negative_count = 0
         seen_neutral_strengthner = False
         negation_words_count = 0
         for clause in clauses:
             negating = False
             tokens = clause.split()
             for token in tokens:
                 t = token.lower().strip(string.punctuation)
                 if t in neutral_strengthner:
                   seen_neutral_strengthner = True
   ...
   def find_movies_closest_to_title(self, title, max_distance=3):
      ...
      for i in range(len(self.titles)):
          cur_title = self.titles[i][0][:-7].lower()
          for ending in stop_words:
              if cur_title[-(len(ending) + 2):] == ", " + ending:
                  cur_title = f"{ending} {cur_title[:-len(ending) - 2]}"
                  break
       
          A = np.zeros( (len(title)+1, len(cur_title)+1)) #initializes matrix with appropriate dimensions, but with all-zero values

          for j in range (len(title)+1): #fills in values for mispelled title column
              A[j][0] = j

          for k in range (len(cur_title)+1): #fills in values for movie title row
              A[0][k] = k

          for l in range (1, len(title)+1):
              for o in range (1, len(cur_title)+1):
          ...
          edit_dis = A[len(title)][len(cur_title)]
        
          if (edit_dis < min_dis):
              min_list = [self.find_movies_by_title(cur_title)[0]]
              min_dis = edit_dis
          elif edit_dis == min_dis:
   ...
   def disambiguate(self, clarification, candidates):
       ...
       for candidate in candidates:
           full = self.titles[candidate][0].lower()
           date = full[-6:].strip("()")
           date_with_parentheses = full[-6:]
           title = full[:-6].lower()
           if clarification == full:
               if full not in self.new_candidates_titles:

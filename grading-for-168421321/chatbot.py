...
class Chatbot:
    ...
    def __init__(self, creative=False):
        ...
        self.mispelled_line = ""
        self.mult_sent_mode = False
        self.clar_mode = False
        self.candidates = []
        self.clar_line = ""
    ...
    def process(self, line):
        ...
        pos_sentiment = ["Marvelous! You enjoyed ", "Let me jot that down in my fieldbook. You liked ",
                         "Another piece to the puzzle! You enjoyed ", "Splendid! You're a fan of ", "Astonishing fact! You liked "]
        # Bank for negative sentiment
        neg_sentiment = ["Noted. You are not a fan of ", "I observe that you did not like ",
                         "Another clue to the case of the lost recommendation! You disliked ", "Preposterous! You didn't like "]
        # Bank for non-existent movie
        non_existent_movie = ["In all my years as a detective, I've never come across ",
                              "I'm afraid I haven't received data about ", "Never heard of ", "Hmmm. Not a clue about "]
        
        fail_gracefully = ["After careful examination, there appear to be no movies in the clue you've given me. ", "Such an expression is devoid of movies. "]
        ...
        for question_phrase in question_bank:
            if line.lower().startswith(question_phrase):
                return response + question_bank[question_phrase] + line + "??" + " Anyway, that's not really what I want to talk about right now." + random.choice(more_ratings)
        ...
        if(self.clar_mode):
            final_titles = self.disambiguate(line, self.candidates)
            if(len(final_titles) != 1):
                self.clar_mode = False
                return response + " Data! Data! Data! I'm not sure which movie you are referring to. " + random.choice(more_ratings)
            line = self.clar_line
            
            possible_titles = [self.titles[final_titles[0]][0]]

        # If no matching title: fail gracefully
        if len(possible_titles) == 0 and not self.clar_mode:
            # Creative
            emotion = self.extract_emotion(line)
            if (emotion[0] != "" and self.creative):
                addendum = "My sincerest apologies. " if emotion[1] < 0 else "Wow that's great! "
                return response + "Did I make you feel " + emotion[0] + "? " + addendum + random.choice(more_ratings)
            return response +  random.choice(fail_gracefully) + random.choice(more_ratings)
        # If many possible titles: fail gracefully
        if len(possible_titles) > 1 and not self.clar_mode:
            if (self.creative):
                sentiments = self.extract_sentiment_for_movies(line)
                if len(sentiments) == 0:
                    return response + "Sorry about that, can't quite ascertain how you feel about those movies!"
                else:
                    for tup in sentiments:
                        ...
                        if sentiment == 0:
                            response += response + " " + random.choice(no_sentiment) + '"' + movie + "\" "
                        # If positive/negative: add sentiment to that index in self.user_ratings, increment self.user_ratings_count
                        if sentiment >= 1:
                            response += \
                                random.choice(pos_sentiment) + '"' + movie + "\" "
                            self.user_ratings[self.find_movies_by_title(movie)[0]] = 1 if sentiment >= 1 else -1
                            self.user_ratings_count += 1
                        else:
                            response += \
                                random.choice(neg_sentiment) + '"' + movie + "\" "
                            self.user_ratings[self.find_movies_by_title(movie)[0]] = 1 if sentiment >= 1 else -1
                            self.user_ratings_count += 1
                        if (tup != sentiments[-1]):
                            response += "and... "
        ...
        if len(movie_titles) == 0:
            mispelled_title = self.find_movies_closest_to_title(
                possible_titles[0])
            if (mispelled_title):
                self.mispelling_mode = True
                self.mispelled_movie = self.titles[mispelled_title[0]][0]
                self.mispelled_line = line.replace(
                    '"' + possible_titles[0] + '"', "")
        ...
        if len(movie_titles) > 1 and self.creative:
            self.clar_mode = True
            self.clar_line = line
            self.candidates = movie_titles
            movie_titles_str = ""
            for title in movie_titles:
                movie_titles_str += '"' + self.titles[title][0] + '"'
    ...
    def find_movies_by_title(self, title):
        ...
        if (self.creative):
            ...
            for index, val in enumerate(self.titles):
                ...
                all_found = True if len(title_words) > 0 else False
    ...
    def extract_emotion(self, line):
        emotions = ['tired', 'happy', 'sad', 'angry', 'surprised', 'disgusted', 'fearful', 'content', 'excited', 'hopeful', 'confused', 'embarrassed', 'lonely', 'nostalgic', 'grateful', 'envious', 'jealous', 'guilty', 'ashamed', 'proud', 'amused', 'bored', 'curious', 'ecstatic', 'exhausted', 'furious', 'gloomy', 'grumpy', 'joyful', 'miserable', 'optimistic', 'pessimistic', 'regretful', 'relieved', 'skeptical', 'sympathetic', 'tense', 'worried']
    ...
    def remove_titles_without_quotation(self, sentence):
        movie_titles = self.extract_titles(sentence)
        for title in movie_titles:
            sentence = sentence.replace(title, "")
    ...
    def extract_sentiment_for_movies(self, preprocessed_input):
        ...
        line = self.remove_titles_without_quotation(preprocessed_input).lower()
        line = line.strip()
        ...
        connecting_words = ['but', 'whereas', 'while']

        for index, phrase in enumerate(line_split):
        ...
        for index, phrase in enumerate(line_split):
            curr_sent = self.extract_sentiment(phrase)

            if (curr_sent == 0 and index > 0):
                curr_sent = sentiments[index - 1]
                for word in negation_words:
                    if word in phrase:
                        curr_sent = sentiments[index - 1] * -1
            sentiments.append(curr_sent)
        ret = []

        if (len(sentiments) != len(movie_titles)):
            return []
        for index, val in enumerate(sentiments):
            tup = (movie_titles[index], val)
            ret.append(tup)
    ...
    def disambiguate(self, clarification, candidates):
        ...
        if clarification.isnumeric() and int(clarification) <= len(plausible_movie_titles):
            return [candidates[int(clarification) -1]]
        for movie_title in plausible_movie_titles:
            
            if clarification.lower().replace(".", "") in movie_title[0].lower():
    ...
    def debug(self, line):
        ...
        your evaluators.
        """
        debug_info = 'error processing the following line ' + line
        return debug_info
    ...
    def intro(self):
        ...
        Description: I am a chatbot name Sherlock HomieBot and I can provide you with 
        movie recommendations based on how you feel about movies. I can also do a couple of 
        other cool things! Tell me about movies you've seen!
        """

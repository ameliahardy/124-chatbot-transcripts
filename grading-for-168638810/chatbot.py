...
class Chatbot:
    ...
    def process(self, line):
        ...
        titles = self.extract_titles(line)
        before = self.num_ratings
        self.num_ratings += len(titles)
        if before < 5 and self.num_ratings >= 5:
            is_recommend = True
    ...
    def find_movies_by_title(self, title):
        ...
        for i in range(len(self.titles)):
            ...
            if movie[:-6].strip() == title_new:
                if year == '' or year == movie.split(' ')[-1]:
                    titles.append(i)
                    found = True
    ...
    def extract_sentiment(self, preprocessed_input):
        ...
        """
        # To-Do: See what constitutes very negative or very positive sentiment
        sentiment = 0 # neutral
    ...
    def find_movies_closest_to_title(self, title, max_distance=3):
        ...
        title = title.lower()
        # title = re.sub('[{}]'.format(string.punctuation), '', title) # remove punctuation
        # title = re.sub('\((\d+)\)', '', title) # remove year
        
        for i in range(len(self.titles)):
            movie_title = self.titles[i][0].lower()
            # movie_title = re.sub(r'[{}]'.format(string.punctuation), '', movie_title) # remove punctuation
            # movie_title = re.sub('\((\d+)\)', '', movie_title) # remove year
